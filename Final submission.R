...
#Import data set 
library(readr)
Food_demand <- read_csv("data/Food demand.csv", col_types = cols(
  homepage_featured = col_factor(levels = c("0","1")),
  emailer_for_promotion = col_factor(levels = c("0","1")),
  num_orders = col_double()
))
View(Food_demand)
...
food_demand_homepage_featured_freq <- Food_demand$homepage_featured
cbind(frequency = table(food_demand_freq),
      percentage = prop.table(table(food_demand_freq)) * 100)
...
#Mode
food_demand_number_orders_mode <- names(table(Food_demand$num_orders))[
  which(table(Food_demand$num_orders) == max(table(Food_demand$num_orders)))
]
...
#standard deviation
sapply(Food_demand[, c(5,6)], sd)
#variance
sapply(Food_demand[, c(5,6)], var)
#Kurtosis
sapply(Food_demand[, c(5,6)], kurtosis, type =2)
#skewness
...
#covariance
food_demand_cov <- cov(Food_demand[, c(5,6)])
View(food_demand_cor)


#correlation
food_demand_cor <- cor(Food_demand[, c(5,6)])
View(food_demand_cor)
...
hist(Food_demand$num_orders, main = "Histogram of Data", xlab = "Values", col = "skyblue", border = "black")
boxplot(Food_demand$num_orders, main = "Boxplot of Data", xlab = "Values", col = "lightgreen", border = "black")


data <- data.frame(x = Food_demand$base_price, y = Food_demand$checkout_price)

# Create a ggplot scatter plot
ggplot(data, aes(x = x, y = y)) +
  geom_point(color = "blue") +
  labs(title = "Scatter Plot of Base price against checkout price", x = "Base price", y = "Checkout price")

# Sample data with two variables
data <- data.frame(x = Food_demand$checkout_price, y = Food_demand$num_orders)

# Create a boxplot for each variable
boxplot(data, main = "Boxplot of checkout price and number of orders", col = c("lightblue", "lightgreen"), border = "black")
...
model_of_the_transform <- preProcess(Food_demand,
                                     method = c("scale", "center"))
print(model_of_the_transform)
food_demand_standardize_transform <- predict(model_of_the_transform, # nolint
                                                Food_demand)
...
food_demand_dataset_model_lm <- # nolint
  caret::train(num_orders ~ .,
               data = food_demand_train,
...
print(food_demand_dataset_model_lm)
...
predictions_lm <- predict(food_dateset_model_lm, food_demand_test[, 1:9])

### 2.c. View the RMSE and the predicted values ====
print(food_dateset_model_lm)
print(predictions_lm)
...
food_demand_caret_model_lm <- train(num_orders ~ ., data = Food_demand_train,
                                       method = "lm", metric = "RMSE",
...
saveRDS(food_demand_caret_model_lm, "./saved_food_demand_caret_model_lda.rds")
